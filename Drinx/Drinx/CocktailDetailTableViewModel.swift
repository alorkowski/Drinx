...
extension CocktailDetailTableViewModel {
    func saveCocktailToFavorites() {
        guard let cocktail = self.cocktail,
            !self.cocktailController.savedCocktails.contains(cocktail)
            else { return }
        self.cocktailController.savedCocktails.append(cocktail)
        self.cocktailController.saveMyFavoriteCocktailsToUserDefaults()
    }
}
